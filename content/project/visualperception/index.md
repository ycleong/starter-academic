...
Analyzing participants’ behavior using a computational model of the underlying decision processes, I further showed that participants’ motivational bias could be decomposed into an a priori bias to respond in a motivation-consistent manner, as well as a bias in the accumulation of incoming sensory evidence. The model-estimated bias in evidence accumulation tracked modulations in category-specific neural activity in the visual cortex, providing converging evidence that motivation biased perceptual processing. Together, these results not only demonstrate that motivation biases perception, but also provide a neurocomputational account of how this occurs.
